...
const latestAppointments = appointments
        .sort((a, b) => new Date(b.time) - new Date(a.time)) // Sort appointments by time (latest first)
        .slice(0, 5); // Get the first 5 appointments
  ...
  return (
    <div className='p-10 bg-[#f8f9fd] w-full'>
        ...
        <div className="mt-6">
            <h2 className="text-[20px] text-[#323232] font-[600] mb-3">Latest Appointments</h2>
            {latestAppointments.map((appointment, index) => {
                const user = users.find((u) => u.id === appointment.user_id);
                const doctor = doctors.find((d) => d.id === appointment.doctor_id);
                return (
                    <div key={index} className="bg-white p-4 mb-4 shadow-md rounded-[6px] flex gap-4 items-center">
                        <div className="flex flex-col">
                            <p className="text-[18px] text-[#515151]">{user?.name}</p>
