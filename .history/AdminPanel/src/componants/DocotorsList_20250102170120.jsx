...
const [data,setData]=useState([])
  ...
  return (
    <div>
        <p>All Doctors</p>
        <div className='grid grid-cols-5 rounded-b-[12px] border-[1px] border-[#C9D8FF]'>
            {data.map((item,index)=>(
                <dir >
                 <img className='w-[200px] h-[216px] bg-[#EAEFFF] rounded-t-[12px]' src={`http://localhost:3000/${item.image}`} alt="" />
                 <p>{item.name}</p>
                 <p>{item.speciality}</p>
