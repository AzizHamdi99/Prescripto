                ...
                return (
                    <div key={index} className="bg-white p-4  flex gap-4 items-center border-[1px] border-[#D8D8D8]">
                        <div className="flex flex-col">
                            ...
                            <p className="text-[16px] text-[#696B80]">{doctor?.speciality}</p>
                            <div className='flex gap-1'>
                            <p className="text-[16px] text-[#696B80]">{appointment.date} |</p>
                            <p className="text-[16px] text-[#696B80]">{appointment.time}</p>
                            </div>
                        </div>
                        <div className="flex items-center">
                            <img className="h-[40px] w-[40px] rounded-full bg-[#EAEFFF]" src={`http://localhost:3000/${doctor?.image}`} alt={doctor?.name} />
                            <p className="ml-2 text-[18px] text-[#262626]">{doctor?.name}</p>
                        </div>
                    </div>
                );
            })}
